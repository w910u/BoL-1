...
if ValidTarget(Target) then
	if Fury <= 4 then
		...
		if eReady and GetDistance(Target) < 1000 and GetDistance(Target) > 300 then
			CastSpell(_E, Target)
		end
...
function TripleQ()
	...
	if Phase == 1 then
		if not qReady or not rReady or Fury < 4 then
			TqText = "Need Q + R and 4 or more fury!"
            if ValidTarget(Target) then
                Combo()
            end
            return
		elseif rReady and qReady and Fury == 4 then
			DisableAttacks = true
			CastSpell(_R)
			Phase = 2
			EndTime = GetGameTimer() + 6
			
		elseif rReady and qReady and Fury == 5 then
			DisableAttacks = true
			CastSpell(_R)
			CastSpell(_Q)
			Phase = 3
	...
	if Phase == 3 then
		...
		if DisableAttacks == false and timeToShoot() then
			CastSpell(_Q)
...
--				DrawText("|",13,barPos.x+IndicatorPos ,barPos.y-18 ,ARGB(255,0,255,0))
	...
	if RMenu.ShowE then
            if eReady then
                DrawCircle(myHero.x, myHero.y, myHero.z, 1000, ARGB(255,0,255,0))
            else
...
--	gragasbarrelrolltoggle
	if object == myHero then
		...
		if spell.name:find("RengarQ") and Phase >= 2 then
			qCount = qCount + 1
